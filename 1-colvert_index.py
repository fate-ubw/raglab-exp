...
if __name__=='__main__':
    dataroot = '/home/wyd/raglab-exp/data/retrieval/colbertv2.0_passages/lotte'
    dataset = 'lifestyle'
    datasplit = 'dev'
    queries = os.path.join(dataroot, dataset, datasplit, 'questions.search.tsv')
    collection = os.path.join(dataroot, dataset, datasplit, 'collection.tsv')

    queries = Queries(path=queries) # Q: 这个类是干啥的呢？？？？，其实就是简单的
    ...
    checkpoint = '/home/wyd/raglab-exp/model/colbertv2.0'
    index_name = f'{dataset}.{datasplit}.{nbits}bits'
    # Q：这个框架是否实现了自查有没有 idnex 函数，
    with Run().context(RunConfig(nranks=1, experiment='/home/wyd/raglab-exp/data/retrieval/colbert_test')):  # nranks specifies the number of GPUs to use.
    ...
    with Run().context(RunConfig(experiment='/home/wyd/raglab-exp/data/retrieval/colbert_test')):
